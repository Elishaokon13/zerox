...
export async function GET() {
  ...
  // Get leaderboard data (without lifetime tracking for now to avoid join issues)
  ...
  const top = (data || []).map((r: { 
    ...
    losses: r.losses, 
    points: r.points,
    lifetimeEarnedUsdc: 0, // Will be added back later when lifetime tracking is populated
    isCapped: false, // Will be added back later when lifetime tracking is populated
    cappedAt: null, // Will be added back later when lifetime tracking is populated
